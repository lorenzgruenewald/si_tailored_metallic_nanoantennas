...
def gendecks(data, key, name):
    ...
    with open('template_parabola.3d', 'r') as f:
        ...
        for i in data:
            ...
            parameters["antenna_xmax"] = str(complex(-complex(parameters["antenna_L"])*(complex(parameters["antenna_rmin"])**2)/(complex(parameters["antenna_rmax"])**2-complex(parameters["antenna_rmin"])**2)+complex(parameters["parabola_base_point"])+complex(parameters["delta_L"])).real)
    ...
    for k in range(nfiles):
        ...
        batch = {"name": f"Parabola_closing_Scan_{k}",
                 "nodenumber": f"{njobs}",
                 "ntasks": "48",
                 "jobnames": "\n".join(listjobs),
